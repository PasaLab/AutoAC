...
class FixedNet_All(nn.Module):
    def __init__(self, data_info, idx_info, train_info, inner_data_info, gnn_model_manager, alpha, args):
        super(FixedNet_All, self).__init__()
    ...
    def _construct_mask_matrix(self):
        ...
        for i in range(len(PRIMITIVES)):
            unattributed_id_arr = np.where(self.arch_indices == i)[0]
            origin_id_arr = [self.unAttributedID2nodeID[x] for x in unattributed_id_arr]
