...
class Network_Darts(nn.Module):
    # def __init__(self, features_list, adjM, labels, train_val_test, dl, in_dims, num_classes, args):
    ...
    def new(self):
        model_copy = Network_Darts(self.g, self._criterion, self.train_val_test, self.type_mask, self.dl, self.in_dims, self.num_classes, self.args, self.e_feat).cuda()
