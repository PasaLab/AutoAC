...
from fixed_net_all_nodes import FixedNet_All

from utils import *
from .supernet import Network_Nasp_Allnodes

class NASPSearcherAllnodes:
    def __init__(self, data_info, idx_info, train_info, gnn_model_manager, args):
    # def __init__(self, args):
        super(NASPSearcherAllnodes, self).__init__()
        ...
        self._supernet = Network_Nasp_Allnodes(data_info, idx_info, train_info, gnn_model_manager, args)
    ...
    def _save_search_info(self):
        # save_path_name = os.path.join('disrete_arch_info', self._save_dir_name + '_' + self.args.dataset + '_' +  str(self.args.cur_repeat))
        # save_path_name = os.path.join('disrete_arch_info', self._save_dir_name + '_' + self.args.dataset)
    ...
    def _train_search(self):
        ...
        if self.args.use_minibatch is False:
            ...
            for step in range(train_idx_generator.num_iterations()):
                ...
                self._supernet.step(self.all_input, self.all_target, (val_g_list, val_indices_list, val_idx_batch_mapped_list, val_idx_batch), lr, self._arch_optimizer, _node_embedding)
            ...
            return train_loss_avg, node_embedding, lr
    ...
    def create_retrain_model(self, alpha):
        inner_data_info = self._supernet.gnn_model_manager.get_graph_info()
        gnn_model_manager = self._supernet.gnn_model_manager
        model = FixedNet_All(self.data_info, self.idx_info, self.train_info, inner_data_info, gnn_model_manager, alpha, self.args)
